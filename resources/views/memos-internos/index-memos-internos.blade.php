...
@section('content')
    <h3 class="mb-4">Memorandums Internos</h3>
        ...
        <table id="tabla" class="table table-striped table-hover table-bordered table-sm shadow">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-info btn-sm mb-1" href="{{ route('memos-internos.show' , ['memos_interno' => $memo->id])}}">Ver</a>
                        @if (Auth::user()->id == $memo->id_usuario_adiciono)
                            <form action="{{ route('memos-internos.destroy' , ['memos_interno' => $memo->id])}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <a class="btn btn-success btn-sm mb-1" href="{{ route('memos-internos.edit' , ['memos_interno' => $memo->id])}}">Modificar</a>
    ...
    @else
        <br><span class="badge bg-secondary">No hay memorandums internos registrados</span>
    @endif
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitulo">Registrar nuevo memorandum interno</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('memos-internos.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
