...
@section('content')
<h3 class="my-4">Detalles del memorandum interno</h3>
...
@if (Auth::user()->id == $memos->id_usuario_adiciono)
    <form action="{{ route('memos-internos.destroy' , ['memos_interno' => $memos->id])}}" method="POST">
        @csrf
        @method('DELETE')
        <a class="btn btn-success" href="{{ route('memos-internos.edit' , ['memos_interno' => $memos->id])}}">Modificar</a>
