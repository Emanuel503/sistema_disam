...
@section('content')
    <h3 class="my-4">Modificar memorandum externos</h3>

    <a class="btn btn-outline-secondary mb-4" href="{{ route('memos-externos.index')}}">Regresar</a>

    @include('layouts.alerts')

    <form action="{{ route('memos-externos.update', ['memos_externo' => $memos->id]) }}" method="POST">
        @csrf
        @method('PATCH')
        <div class="mb-3">
        ...
        <a href="{{route('memos-externos.index')}}" class="btn btn-secondary mt-4">Cancelar</a>
    </form>
@endsection